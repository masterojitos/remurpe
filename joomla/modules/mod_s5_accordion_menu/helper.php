...
class JMenuTree2 extends JTree
{
	...
	function _getLevelXML($depth)
	{
		...
		if ($depth <= 1) {
			$this->_buffer .= '<h3 class="s5_am_toggler" access="'.$this->_current->access.'" level="'.$depth.'" id="'.$this->_current->id.'"'.$rel.'><span class="s5_accordion_menu_left"></span>';} 
		else {
		...
		if ($tmp->url != null)
		{
			...
			if ($tmp->home == 1) {
				...
				switch ($tmp->browserNav)
				{
					...
					case 0:
						// _top
						$data = '<a class="mainlevel" href="'.$tmp->url.'">'.$image.$tmp->name.'</a>';
						break;
					case 1:
						// _blank
						$data = '<a class="mainlevel" href="'.$tmp->url.'" target="_blank">'.$image.$tmp->name.'</a>';
						break;
					case 2:
						// window.open
