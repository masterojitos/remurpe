	...
	function getImages($text, $thumb_size=70) {	  
		...
		if (isset($matches[1])) {
			...
			if (function_exists("gd_info")) {
				...
				if (file_exists(JPATH_SITE.'/'.$thumb_path)) {
				    $existing_thumb = new Thumbnail(JPATH_SITE.'/'.$thumb_path);
				    $current_size = $existing_thumb->getCurrentWidth();
					$images->thumbSizes = $existing_thumb->currentDimensions;
				}

                if (!file_exists(JPATH_SITE.'/'.$thumb_path) || $current_size!=$thumb_size) {
    				...
    				if (!is_writable(dirname(JPATH_SITE.'/'.$thumb_path))) {
    					$thumb->destruct();
    					return false;
    				}
					
    				$thumb->save(JPATH_SITE.'/'.$thumb_path);
					$images->thumbSizes = $thumb->currentDimensions;
    				$thumb->destruct();
    			}
			}
			$images->thumb = JURI::Root(True)."/".$thumb_path;
		} 
		return $images;
	}
