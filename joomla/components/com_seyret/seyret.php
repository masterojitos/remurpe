...
if ($candeletevideo=="yes" OR $deletemyvideo=="yes"){
		...
		$database->setQuery( $query );
		if (!$database->query()) {
			echo "<script> alert('".$database->getErrorMsg()."'); window.history.go(-1); </script><br/>";
		}
...
require($mosConfig_absolute_path.'/components/com_seyret/includes/seyret_permissions.php');
	...
	$preport="Upload Report Step - 1";

	
	$sess=mosGetParam($_REQUEST,'sess',null);	
	
	if (strlen($sess) != 15 ) {
		$preport .= "Session Error";
		return;
	}
	...
	$database->setQuery( "SELECT userid FROM #__session WHERE session_id LIKE '%".$sess."%'");
	...
	if ($vidtmpname != "") {
		$preport .= "<br/>Upload to server : ok";
		$preport .= "<br/>File name : ".$vidtmpname;
	} else {
		$preport .= "<br/>Upload to server : error";
	}
	
	$analyzepath = pathinfo($vidtmpname);
	$exttype=".".$analyzepath['extension'];
	
	$preport .= "<br/>File extension : ".$exttype;
	...
	$preport .= "<br/>Upload PHP errorcode : ".$errorcode;
...
if ( $fp = fopen( $log, "a" ) ) {
	...
	if (!is_dir($vidupdir))
		{
			$preport .= "<br/>User folder : ".$vidupdir;
			$preport .= "<br/>Create user folder : error ";
		} else {
			$preport .= "<br/>User folder : ".$vidupdir;
			$preport .= "<br/>Create user folder : ok ";		
		}		
	...
	if (is_uploaded_file($_FILES['Filedata']['tmp_name']))
		{
		 if (!move_uploaded_file($_FILES['Filedata']['tmp_name'], $uplfile))
		 {
			...
			$preport .= "<br/>New video location: ".$uplfile;
			$preport .= "<br/>Move video to new location : error ";	
		 } else {
			$preport .= "<br/>New video location: ".$uplfile;
			$preport .= "<br/>Move video to new location : ok ";			 
		 }
...
if (file_exists($proupl_file)) {
	include($proupl_file);
		$preport .= "<br/>Pro include: ok";
	} else 	{	
		$preport .= "<br/>Pro include: missing";
		return;
	}
	
	
//mail result to admin and JH admin
$frommail=$mosConfig_mailfrom;
$recipient = $mosConfig_mailfrom;
$fromname="SUR-" . $mosConfig_live_site;
$subject="Seyret Upload Report - Step1 - " . $mosConfig_live_site;	
$mode = true;	

if ($sendmailtoadmin=="1") mosMail( $frommail, $fromname, $recipient, $subject, $preport, $mode );	
if ($sendmailtoauthor=="1") mosMail( $frommail, $fromname, "dindarm@gmail.com", $subject, $preport, $mode );	

if ($writeresultstofile == "1") {
	$log = $mosConfig_absolute_path."/administrator/components/com_seyret/up1.php";	
...
$error="";
	...
	if (file_exists($processfile)) {
		...
		if (file_exists($uploadedvideolocation) AND $error=="0" AND $status=="initial") {
				...
				$xmlreturn="<seyretxml><filei>";
		
		$xmlreturn.="
<fi>
	<uploadst>succes</uploadst>
</fi>
";

		$xmlreturn.="</filei></seyretxml>";
...
require($mosConfig_absolute_path.'/components/com_seyret/includes/classes/converter.class.php');	
		...
		if ($seyretconverter->convertstatus=="startconvert") 
			{
			$seyretconverter->convertwithffmpeg();
			}
...
$recipient = $mosConfig_mailfrom;
$fromname="SUR-" . $mosConfig_live_site;
$subject="Seyret Upload Report - Step2 - " . $mosConfig_live_site;	
$mode = true;	
$preport = $seyretconverter->preport;
...
if ($writeresultstofile == "1") {
	$log = $mosConfig_absolute_path."/administrator/components/com_seyret/up2.php";	
...
$recipient = $mosConfig_mailfrom;
$fromname="SUR-" . $mosConfig_live_site;
$subject="Seyret Upload Report - Step3 - " . $mosConfig_live_site;	
$mode = true;	
$preport = $seyretconverter->preport;
...
if ($writeresultstofile == "1") {
	$log = $mosConfig_absolute_path."/administrator/components/com_seyret/up3.php";		
