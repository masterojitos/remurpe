...
class seyretconverter{
	...
	function convertwithffmpeg(){
		...
		if (function_exists("shell_exec")) {
			$this->preport .= "<br/>shell_exec function found: ok";
		} else {
			$this->preport .= "<br/>shell_exec function not found: error";
		}
		...
		if (file_exists($this->videooutputfile)) {
			$this->preport .= "<br/>Output file: ".$this->videooutputfile;
			$this->preport .= "<br/>Output file found: ok";
		} else {
			$this->preport .= "<br/>Output file: ".$this->videooutputfile;
			$this->preport .= "<br/>Output file not found: error";
		}
	...
	function checkconversionconfig(){
		...
		require($mosConfig_absolute_path.'/administrator/components/com_seyret/seyret_config.php');
			...
			if ($enableflvconversion=="1" AND $instantvideoprocess=="1"){
				...
				if ($this->checkfileexists($ffmpegpath, "ffmpeg")) {
					$this->preport .= "<br/>Instant conversion settings: ok";
					$this->preport .= "<br/>Ffmpeg path:". $ffmpegpath;
					$this->preport .= "<br/>Ffmpeg found: ok";
				} else {
					$this->preport .= "<br/>Instant conversion settings ok";
					$this->preport .= "<br/>Ffmpeg path:". $ffmpegpath;
					$this->preport .= "<br/>Ffmpeg not found. Check your seyret config. Error";
				}
				
				if ($this->checkfileexists($this->inputfile, "inputfile")) {
					$this->preport .= "<br/>Input file found: ok";					
				} else {
					$this->preport .= "<br/>Input file not found. Probably upload failure. Error.";
				};
	...
	function checkthumbnailextractconfig(){
		...
		require($mosConfig_absolute_path.'/administrator/components/com_seyret/seyret_config.php');
			...
			if ($enablethumbnailextraction=="1" AND $instantthumbprocess=="1"){
				...
				if ($this->checkfileexists($ffmpegpath, "ffmpeg")) {
					$this->preport .= "<br/>Thumbnail extract settings: ok";
					$this->preport .= "<br/>Ffmpeg path:". $ffmpegpath;
					$this->preport .= "<br/>Ffmpeg found: ok";
				} else {
					$this->preport .= "<br/>Thumbnail extract settings: ok";
				...
				if ($this->checkfileexists($this->inputfile, "inputfile")) {
					$this->preport .= "<br/>Input file path: ". $this->inputfile ;					
					$this->preport .= "<br/>Input file found: ok";					
				} else {
					$this->preport .= "<br/>Input file path: ". $this->inputfile ;	
					$this->preport .= "<br/>Input file not found. Probably upload failure. Error.";
