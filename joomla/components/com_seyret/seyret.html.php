...
<?php
	...
	if (file_exists($frontpagecachefile)) {
		...
		$template= jalemseyret_read_file($templatefile)."<div id=\"hiddenvalues\" style=\"display:none;\"></div>";
	...
	if ($useimageadsystem=="1") {
		$videoembedarea="<div name=\"mediaspace\" id=\"mediaspace\" style=\"min-height:".$videoheight."px; min-width:".$videowidth."\"   class=\"videoarea\">".$imagead."</div>";
	} else  {
		$videoembedarea="<div name=\"mediaspace\" id=\"mediaspace\"  style=\"min-height:".$videoheight."px;\"  min-width:".$videowidth."\" class=\"videoarea\">".$randomfeaturedvideo."</div>";
	}
...
list ($embed, $voperations, $votearea, $embedcode, $videodirectlink, $comments, $imagead, $addedby, $videotagslinked) = generatecodes($vtype, $vcode, $id, $accesslevel, $frombackupid, $addedby, $isdownloadable, $vthumb, $catid, $videotags, $videourl);
	...
	//read main template
	$templatefile=$mosConfig_absolute_path."/components/com_seyret/themes/".$theme."/templates/frontpage.php";
	$template= jalemseyret_read_file($templatefile)."<div id=\"hiddenvalues\" style=\"display:none;\"><div id=\"hiddenthevideoid\">".$id."</div></div>";
	...
	if ($useimageadsystem=="1") {
	$videoembedarea="<div name=\"mediaspace\" id=\"mediaspace\" style=\"min-height:".$videoheight."px;  min-width:".$videowidth."px;\" class=\"videoarea\">".$imagead."</div>";
	} else {
	$videoembedarea="<div name=\"mediaspace\" id=\"mediaspace\" style=\"min-height:".$videoheight."px;  min-width:".$videowidth."px;\"  class=\"videoarea\" >".$embed."</div>";
	}
...
<br>
	...
	<div id="videouploader" style="border:none;">
	<object  id="videouploader"  name="videouploader"  width="470" height="170" > 
	<param name="wmode" value="transparent" /> 
	<param name="movie" value="<?php echo $mosConfig_live_site; ?>"/components/com_seyret/videouploader.swf" />
	<param name="flashvars" value="siteurl=<?echo $mosConfig_live_site;?>&sess=<?php echo $sid; ?>&uid=<?php echo $my->id;?>" />
	
	<embed id="videouploader" name="videouploader" src="<?php echo $mosConfig_live_site; ?>/components/com_seyret/videouploader.swf" 
	flashvars="siteurl=<?echo $mosConfig_live_site;?>&sess=<?php echo $sid; ?>&uid=<?php echo $my->id;?>" width="470" height="170"   wmode="transparent"  type="application/x-shockwave-flash" /></object>
	</div>
...
$step=10;
	...
	$database->setQuery("SELECT * FROM #__seyret_items WHERE published='1' ORDER BY addeddate DESC LIMIT $navfrom, $step");
			...
			foreach ($allvidlist as $allvidlist) {
				...
				$aiaddeddate=mosFormatDate( $allvidlist->addeddate, "%d %B %Y" );
