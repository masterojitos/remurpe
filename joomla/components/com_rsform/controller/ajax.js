...
function saveRow(id, total)
{
	...
	for(i =0;i<SubmissionValueIds.length;i++)
	{
		...
		//if(value){
			...
			xmlHttpArray[j].open("POST","index2.php?option=com_rsform&task=submissions.edit&action=edit&formId=" + formId + "&SubmissionId=" + id + "&SubmissionValueId=" + SubmissionValueIds[i].value + "&fieldName=" + FieldNames[i].value,true);
