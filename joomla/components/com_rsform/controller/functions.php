...
defined( '_JEXEC' ) or die( 'Restricted access' );
	...
	function RSfrontLayout($formId, $formLayout)
	{
		...
		return str_replace('{global:formtitle}', $formTitle, $formLayout);
	}
	...
	function RSshowForm($formId,$val='',$validation='')
	{
		...
		$db = JFactory::getDBO();
		$db->setQuery("SELECT `FormId`, `FormLayout`, `ScriptDisplay` FROM #__rsform_forms WHERE FormId='".$formId."' AND `Published`='1'");
		$r = $db->loadAssoc();
		...
		{
			$u = 'index.php?'.$_SERVER['QUERY_STRING'];
		}
		else
		{
			$u = JFactory::getURI();
    ...
    foreach($form_rows as $form_row)
    {
	    ...
	    if($submissions)
		{
		    ...
		    foreach($submission_rows as $submission_row)
		    {
			    ...
			    {
			         $tasks[] = RSxmlReturnQuery('#__rsform_submission_values',$submission_value_row,'SubmissionValueId','SubmissionId');
