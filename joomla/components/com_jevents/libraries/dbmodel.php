 ...
 *
 * @version     $Id: dbmodel.php 1544 2009-08-20 07:53:10Z geraint $
 * @package     JEvents
...
//		$extratables = "";  // must have comma prefix
	...
	function listIcalEventsByYear( $year, $limitstart, $limit, $showrepeats = true, $order="", $filters = false, $extrafields="", $extratables="", $count=false) {
		...
		$thisyear = new JDate();		
		if (!$this->cfg->getValue("showyearpast",1) && $year<$thisyear->toFormat("%Y")){
			return array();	
		}
		$startdate 	= ($this->cfg->getValue("showyearpast",1) || $year>$xyear)?mktime( 0, 0, 0, 1, 1, $year ):mktime( 0, 0, 0, $month,$day, $year );
