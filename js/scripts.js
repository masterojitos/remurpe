$(document).on("ready", function(){
    ...
    var video_popup = function(player, e) {
        ...
        if (is_android && android_version >= 4) {
            $('div#' + player).html(
            '<video autoplay="autoplay" controls autobuffer src="http://' + video_url_base + '/playlist.m3u8"  width="' + video_width + '" height="' + video_height + '"  poster="../../main/upload/webtv/thumbs/' + file + '.png">\\n\
             </video>');
        } else {
            sources = [
                        ...
                        if (video.id != '') {
                            ...
                            '<article>\n\
                                <div class="video" title="' + video.title + '">\n\
                                    ' + link + '\n\
                                    <img src="../../main/upload/webtv/' + video_folder + '/' + client + '/' + video.video_src + video_ext + '">\n\
                                    </a>\n\
                                </div>\n\
                                ' + controls + '\n\
                            </article>';
